    ...
    for(j in 1:length(months)){
      ...
      for(l in 1:length(days)){
        action_url <- paste0("https://api.actionnetwork.com/web/v1/scoreboard/ncaab?bookIds=123,75&date=",years[i],
                             ifelse(nchar(months[j])==1,paste0(0,months[j]),months[j]),ifelse(nchar(days[l])==1,paste0(0,days[l]),days[l]),"&conference=SEC&tournament=0")
        t <- GET(action_url)
...
sec_cbb_udf_analysis <- function(nba_all_games){
...
sec_todays_cbb_analysis <- function(action_all_df){
