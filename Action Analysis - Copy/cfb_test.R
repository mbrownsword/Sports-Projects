...
cfb_backtest <- function(){
  weeks <- c(1:16)
  years <- c(2017:2020)
  action_all_list <-  vector("list",length(years))
  for(i in 1:length(years)){
    action_df_list <- vector("list",length(weeks))
    for(j in 1:length(weeks)){
      action_url <- paste0("https://api.actionnetwork.com/web/v1/scoreboard/ncaaf?bookIds=123,75&season=",years[i],"&week=",
                           weeks[j],"&division=FBS")  
      t <- GET(action_url)
          ...
          if(length(action_odds$games[[u]]$odds)==0){
            ...
            for(k in 1:length(action_odds$games[[u]]$odds)){
              if((action_odds$games[[u]]$odds[[k]]$book_id==123 && action_odds$games[[u]]$odds[[k]]$type=="game") | (action_odds$games[[u]]$odds[[k]]$book_id==75 && action_odds$games[[u]]$odds[[k]]$type=="game")){
                ...
                if(length(action_odds$games[[u]]$odds[[k]]$spread_away_line)==0 || length(action_odds$games[[u]]$odds[[k]]$spread_home_line)==0){
                  action_df$away_spread_line[u] <- "Offline"
                  action_df$home_spread_line[u] <- "Offline"
                }else{
                  action_df$away_spread_line[u] <- action_odds$games[[u]]$odds[[k]]$spread_away_line
                  action_df$home_spread_line[u] <- action_odds$games[[u]]$odds[[k]]$spread_home_line
                }
      ...
      action_df_list[[j]] <- action_df
      print(paste0("Done with week ",weeks[j]," ",years[i]))
    }
  ...
  action_all_df$home_tease <- action_all_df$home_spread+6
  action_all_df$away_tease <- action_all_df$away_spread+6
  action_all_df$under_tease <- action_all_df$total+6
  action_all_df$over_tease <-  action_all_df$total-6
  action_all_df <- action_all_df[-which(is.na(action_all_df$total))]
  action_all_df <- action_all_df[-which(action_all_df$home_ml=="Offline"),]
  
  action_all_df$hut <- 99
  action_all_df$hot <- 99
  action_all_df$aut <- 99
  action_all_df$aot <- 99
  ...
  action_all_df <- action_all_df
}


cfb_udf_analysis <- function(nba_all_games){
...
todays_cfb_analysis <- function(action_all_df, weeks, years){
  ...
  action_all_df_today <- action_all_df_today[order(action_all_df_today$est_time),]
  ...
  for(g in 1:length(nhl_analysis$aroi)){
    sagame_df$aroiMLASHARP[g] <- nhl_analysis$aroi[[g]][1]
    sagame_df$aroiMLHSHARP[g] <- nhl_analysis$aroi[[g]][2]
    sagame_df$aroiSASHARP[g] <- nhl_analysis$aroi[[g]][3]
    sagame_df$aroiSHSHARP[g] <- nhl_analysis$aroi[[g]][4]
    pagame_df$aroiAPUBLIC[g] <- nhl_analysis$aroi[[g]][5]
    pagame_df$aroiHPUBLIC[g] <- nhl_analysis$aroi[[g]][6]
    pagame_df$aroiAMLPUBLIC[g] <- nhl_analysis$aroi[[g]][7]
    pagame_df$aroiHMLPUBLIC[g] <- nhl_analysis$aroi[[g]][8]
    magame_df$aroiAMONEY[g] <- nhl_analysis$aroi[[g]][9]
    magame_df$aroiHMONEY[g] <- nhl_analysis$aroi[[g]][10]
    magame_df$aroiAMLMONEY[g] <- nhl_analysis$aroi[[g]][11]
    magame_df$aroiHMLMONEY[g] <- nhl_analysis$aroi[[g]][12]
    
    sagame_df$aroinMLASHARP[g] <- nhl_analysis$aroin[[g]][1]
    sagame_df$aroinMLHSHARP[g] <- nhl_analysis$aroin[[g]][2]
    sagame_df$aroinSASHARP[g] <- nhl_analysis$aroin[[g]][3]
    sagame_df$aroinSHSHARP[g] <- nhl_analysis$aroin[[g]][4]
    pagame_df$aroinAPUBLIC[g] <- nhl_analysis$aroin[[g]][5]
    pagame_df$aroinHPUBLIC[g] <- nhl_analysis$aroin[[g]][6]
    pagame_df$aroinAMLPUBLIC[g] <- nhl_analysis$aroin[[g]][7]
    pagame_df$aroinHMLPUBLIC[g] <- nhl_analysis$aroin[[g]][8]
    magame_df$aroinAMONEY[g] <- nhl_analysis$aroin[[g]][9]
    magame_df$aroinHMONEY[g] <- nhl_analysis$aroin[[g]][10]
    magame_df$aroinAMLMONEY[g] <- nhl_analysis$aroin[[g]][11]
    magame_df$aroinHMLMONEY[g] <- nhl_analysis$aroin[[g]][12]
    
    sagame_df$mlaroiMLASHARP[g] <- nhl_analysis$mlaroi[[g]][1]
    sagame_df$mlaroiMLHSHARP[g] <- nhl_analysis$mlaroi[[g]][2]
    sagame_df$mlaroiSASHARP[g] <- nhl_analysis$mlaroi[[g]][3]
    sagame_df$mlaroiSHSHARP[g] <- nhl_analysis$mlaroi[[g]][4]
    pagame_df$mlaroiAPUBLIC[g] <- nhl_analysis$mlaroi[[g]][5]
    pagame_df$mlaroiHPUBLIC[g] <- nhl_analysis$mlaroi[[g]][6]
    pagame_df$mlaroiAMLPUBLIC[g] <- nhl_analysis$mlaroi[[g]][7]
    pagame_df$mlaroiHMLPUBLIC[g] <- nhl_analysis$mlaroi[[g]][8]
    magame_df$mlaroiAMONEY[g] <- nhl_analysis$mlaroi[[g]][9]
    magame_df$mlaroiHMONEY[g] <- nhl_analysis$mlaroi[[g]][10]
    magame_df$mlaroiAMLMONEY[g] <- nhl_analysis$mlaroi[[g]][11]
    magame_df$mlaroiHMLMONEY[g] <- nhl_analysis$mlaroi[[g]][12]
    
    sagame_df$mlaroinMLASHARP[g] <- nhl_analysis$mlaroin[[g]][1]
    sagame_df$mlaroinMLHSHARP[g] <- nhl_analysis$mlaroin[[g]][2]
    sagame_df$mlaroinSASHARP[g] <- nhl_analysis$mlaroin[[g]][3]
    sagame_df$mlaroinSHSHARP[g] <- nhl_analysis$mlaroin[[g]][4]
    pagame_df$mlaroinAPUBLIC[g] <- nhl_analysis$mlaroin[[g]][5]
    pagame_df$mlaroinHPUBLIC[g] <- nhl_analysis$mlaroin[[g]][6]
    pagame_df$mlaroinAMLPUBLIC[g] <- nhl_analysis$mlaroin[[g]][7]
    pagame_df$mlaroinHMLPUBLIC[g] <- nhl_analysis$mlaroin[[g]][8]
    magame_df$mlaroinAMONEY[g] <- nhl_analysis$mlaroin[[g]][9]
    magame_df$mlaroinHMONEY[g] <- nhl_analysis$mlaroin[[g]][10]
    magame_df$mlaroinAMLMONEY[g] <- nhl_analysis$mlaroin[[g]][11]
    magame_df$mlaroinHMLMONEY[g] <- nhl_analysis$mlaroin[[g]][12]
  }
  ...
  for(g in 1:length(nhl_analysis$hroi)){
    shgame_df$hroiMLASHARP[g] <- nhl_analysis$hroi[[g]][1]
    shgame_df$hroiMLHSHARP[g] <- nhl_analysis$hroi[[g]][2]
    shgame_df$hroiSASHARP[g] <- nhl_analysis$hroi[[g]][3]
    shgame_df$hroiSHSHARP[g] <- nhl_analysis$hroi[[g]][4]
    phgame_df$hroiAPUBLIC[g] <- nhl_analysis$hroi[[g]][5]
    phgame_df$hroiHPUBLIC[g] <- nhl_analysis$hroi[[g]][6]
    phgame_df$hroiAMLPUBLIC[g] <- nhl_analysis$hroi[[g]][7]
    phgame_df$hroiHMLPUBLIC[g] <- nhl_analysis$hroi[[g]][8]
    mhgame_df$hroiAMONEY[g] <- nhl_analysis$hroi[[g]][9]
    mhgame_df$hroiHMONEY[g] <- nhl_analysis$hroi[[g]][10]
    mhgame_df$hroiAMLMONEY[g] <- nhl_analysis$hroi[[g]][11]
    mhgame_df$hroiHMLMONEY[g] <- nhl_analysis$hroi[[g]][12]
    
    shgame_df$hroinMLASHARP[g] <- nhl_analysis$hroin[[g]][1]
    shgame_df$hroinMLHSHARP[g] <- nhl_analysis$hroin[[g]][2]
    shgame_df$hroinSASHARP[g] <- nhl_analysis$hroin[[g]][3]
    shgame_df$hroinSHSHARP[g] <- nhl_analysis$hroin[[g]][4]
    phgame_df$hroinAPUBLIC[g] <- nhl_analysis$hroin[[g]][5]
    phgame_df$hroinHPUBLIC[g] <- nhl_analysis$hroin[[g]][6]
    phgame_df$hroinAMLPUBLIC[g] <- nhl_analysis$hroin[[g]][7]
    phgame_df$hroinHMLPUBLIC[g] <- nhl_analysis$hroin[[g]][8]
    mhgame_df$hroinAMONEY[g] <- nhl_analysis$hroin[[g]][9]
    mhgame_df$hroinHMONEY[g] <- nhl_analysis$hroin[[g]][10]
    mhgame_df$hroinAMLMONEY[g] <- nhl_analysis$hroin[[g]][11]
    mhgame_df$hroinHMLMONEY[g] <- nhl_analysis$hroin[[g]][12]
    
    shgame_df$mlhroiMLASHARP[g] <- nhl_analysis$mlhroi[[g]][1]
    shgame_df$mlhroiMLHSHARP[g] <- nhl_analysis$mlhroi[[g]][2]
    shgame_df$mlhroiSASHARP[g] <- nhl_analysis$mlhroi[[g]][3]
    shgame_df$mlhroiSHSHARP[g] <- nhl_analysis$mlhroi[[g]][4]
    phgame_df$mlhroiAPUBLIC[g] <- nhl_analysis$mlhroi[[g]][5]
    phgame_df$mlhroiHPUBLIC[g] <- nhl_analysis$mlhroi[[g]][6]
    phgame_df$mlhroiAMLPUBLIC[g] <- nhl_analysis$mlhroi[[g]][7]
    phgame_df$mlhroiHMLPUBLIC[g] <- nhl_analysis$mlhroi[[g]][8]
    mhgame_df$mlhroiAMONEY[g] <- nhl_analysis$mlhroi[[g]][9]
    mhgame_df$mlhroiHMONEY[g] <- nhl_analysis$mlhroi[[g]][10]
    mhgame_df$mlhroiAMLMONEY[g] <- nhl_analysis$mlhroi[[g]][11]
    mhgame_df$mlhroiHMLMONEY[g] <- nhl_analysis$mlhroi[[g]][12]
    
    shgame_df$mlhroinMLASHARP[g] <- nhl_analysis$mlhroin[[g]][1]
    shgame_df$mlhroinMLHSHARP[g] <- nhl_analysis$mlhroin[[g]][2]
    shgame_df$mlhroinSASHARP[g] <- nhl_analysis$mlhroin[[g]][3]
    shgame_df$mlhroinSHSHARP[g] <- nhl_analysis$mlhroin[[g]][4]
    phgame_df$mlhroinAPUBLIC[g] <- nhl_analysis$mlhroin[[g]][5]
    phgame_df$mlhroinHPUBLIC[g] <- nhl_analysis$mlhroin[[g]][6]
    phgame_df$mlhroinAMLPUBLIC[g] <- nhl_analysis$mlhroin[[g]][7]
    phgame_df$mlhroinHMLPUBLIC[g] <- nhl_analysis$mlhroin[[g]][8]
    mhgame_df$mlhroinAMONEY[g] <- nhl_analysis$mlhroin[[g]][9]
    mhgame_df$mlhroinHMONEY[g] <- nhl_analysis$mlhroin[[g]][10]
    mhgame_df$mlhroinAMLMONEY[g] <- nhl_analysis$mlhroin[[g]][11]
    mhgame_df$mlhroinHMLMONEY[g] <- nhl_analysis$mlhroin[[g]][12]
  }
  ...
  cfb_games_today <- action_all_df_today
  cfb_games <- list(sagame_df,
                    pagame_df,
                    magame_df,
                    shgame_df,
                    phgame_df,
