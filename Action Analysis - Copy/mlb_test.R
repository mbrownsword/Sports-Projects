...
mlb_backtest <- function(){
  ...
  for(i in 1:length(years)){
    ...
    for(j in 1:length(months)){
      ...
      for(l in 1:length(days)){
        action_url <- paste0("https://api.actionnetwork.com/web/v1/scoreboard/mlb?bookIds=123,75&date=",years[i],
                             ifelse(nchar(months[j])==1,paste0(0,months[j]),months[j]),ifelse(nchar(days[l])==1,paste0(0,days[l]),days[l]))
        t <- GET(action_url)
  ...
  action_all_df <- action_all_df[-which(is.na(action_all_df$away_score)),]
  ...
  mlb_all_games <- action_all_df
}


mlb_udf_analysis <- function(nba_all_games){
...
todays_mlb_analysis <- function(action_all_df){
  ...
  for(g in 1:length(mlb_analysis$aroi)){
    sagame_df$aroiMLASHARP[g] <- mlb_analysis$aroi[[g]][1]
    sagame_df$aroiMLHSHARP[g] <- mlb_analysis$aroi[[g]][2]
    sagame_df$aroiSASHARP[g] <- mlb_analysis$aroi[[g]][3]
    sagame_df$aroiSHSHARP[g] <- mlb_analysis$aroi[[g]][4]
    pagame_df$aroiAPUBLIC[g] <- mlb_analysis$aroi[[g]][5]
    pagame_df$aroiHPUBLIC[g] <- mlb_analysis$aroi[[g]][6]
    pagame_df$aroiAMLPUBLIC[g] <- mlb_analysis$aroi[[g]][7]
    pagame_df$aroiHMLPUBLIC[g] <- mlb_analysis$aroi[[g]][8]
    magame_df$aroiAMONEY[g] <- mlb_analysis$aroi[[g]][9]
    magame_df$aroiHMONEY[g] <- mlb_analysis$aroi[[g]][10]
    magame_df$aroiAMLMONEY[g] <- mlb_analysis$aroi[[g]][11]
    magame_df$aroiHMLMONEY[g] <- mlb_analysis$aroi[[g]][12]
    
    sagame_df$aroinMLASHARP[g] <- mlb_analysis$aroin[[g]][1]
    sagame_df$aroinMLHSHARP[g] <- mlb_analysis$aroin[[g]][2]
    sagame_df$aroinSASHARP[g] <- mlb_analysis$aroin[[g]][3]
    sagame_df$aroinSHSHARP[g] <- mlb_analysis$aroin[[g]][4]
    pagame_df$aroinAPUBLIC[g] <- mlb_analysis$aroin[[g]][5]
    pagame_df$aroinHPUBLIC[g] <- mlb_analysis$aroin[[g]][6]
    pagame_df$aroinAMLPUBLIC[g] <- mlb_analysis$aroin[[g]][7]
    pagame_df$aroinHMLPUBLIC[g] <- mlb_analysis$aroin[[g]][8]
    magame_df$aroinAMONEY[g] <- mlb_analysis$aroin[[g]][9]
    magame_df$aroinHMONEY[g] <- mlb_analysis$aroin[[g]][10]
    magame_df$aroinAMLMONEY[g] <- mlb_analysis$aroin[[g]][11]
    magame_df$aroinHMLMONEY[g] <- mlb_analysis$aroin[[g]][12]
    
    sagame_df$mlaroiMLASHARP[g] <- mlb_analysis$mlaroi[[g]][1]
    sagame_df$mlaroiMLHSHARP[g] <- mlb_analysis$mlaroi[[g]][2]
    sagame_df$mlaroiSASHARP[g] <- mlb_analysis$mlaroi[[g]][3]
    sagame_df$mlaroiSHSHARP[g] <- mlb_analysis$mlaroi[[g]][4]
    pagame_df$mlaroiAPUBLIC[g] <- mlb_analysis$mlaroi[[g]][5]
    pagame_df$mlaroiHPUBLIC[g] <- mlb_analysis$mlaroi[[g]][6]
    pagame_df$mlaroiAMLPUBLIC[g] <- mlb_analysis$mlaroi[[g]][7]
    pagame_df$mlaroiHMLPUBLIC[g] <- mlb_analysis$mlaroi[[g]][8]
    magame_df$mlaroiAMONEY[g] <- mlb_analysis$mlaroi[[g]][9]
    magame_df$mlaroiHMONEY[g] <- mlb_analysis$mlaroi[[g]][10]
    magame_df$mlaroiAMLMONEY[g] <- mlb_analysis$mlaroi[[g]][11]
    magame_df$mlaroiHMLMONEY[g] <- mlb_analysis$mlaroi[[g]][12]
    
    sagame_df$mlaroinMLASHARP[g] <- mlb_analysis$mlaroin[[g]][1]
    sagame_df$mlaroinMLHSHARP[g] <- mlb_analysis$mlaroin[[g]][2]
    sagame_df$mlaroinSASHARP[g] <- mlb_analysis$mlaroin[[g]][3]
    sagame_df$mlaroinSHSHARP[g] <- mlb_analysis$mlaroin[[g]][4]
    pagame_df$mlaroinAPUBLIC[g] <- mlb_analysis$mlaroin[[g]][5]
    pagame_df$mlaroinHPUBLIC[g] <- mlb_analysis$mlaroin[[g]][6]
    pagame_df$mlaroinAMLPUBLIC[g] <- mlb_analysis$mlaroin[[g]][7]
    pagame_df$mlaroinHMLPUBLIC[g] <- mlb_analysis$mlaroin[[g]][8]
    magame_df$mlaroinAMONEY[g] <- mlb_analysis$mlaroin[[g]][9]
    magame_df$mlaroinHMONEY[g] <- mlb_analysis$mlaroin[[g]][10]
    magame_df$mlaroinAMLMONEY[g] <- mlb_analysis$mlaroin[[g]][11]
    magame_df$mlaroinHMLMONEY[g] <- mlb_analysis$mlaroin[[g]][12]
  }
  ...
  for(g in 1:length(mlb_analysis$hroi)){
    shgame_df$hroiMLASHARP[g] <- mlb_analysis$hroi[[g]][1]
    shgame_df$hroiMLHSHARP[g] <- mlb_analysis$hroi[[g]][2]
    shgame_df$hroiSASHARP[g] <- mlb_analysis$hroi[[g]][3]
    shgame_df$hroiSHSHARP[g] <- mlb_analysis$hroi[[g]][4]
    phgame_df$hroiAPUBLIC[g] <- mlb_analysis$hroi[[g]][5]
    phgame_df$hroiHPUBLIC[g] <- mlb_analysis$hroi[[g]][6]
    phgame_df$hroiAMLPUBLIC[g] <- mlb_analysis$hroi[[g]][7]
    phgame_df$hroiHMLPUBLIC[g] <- mlb_analysis$hroi[[g]][8]
    mhgame_df$hroiAMONEY[g] <- mlb_analysis$hroi[[g]][9]
    mhgame_df$hroiHMONEY[g] <- mlb_analysis$hroi[[g]][10]
    mhgame_df$hroiAMLMONEY[g] <- mlb_analysis$hroi[[g]][11]
    mhgame_df$hroiHMLMONEY[g] <- mlb_analysis$hroi[[g]][12]
    
    shgame_df$hroinMLASHARP[g] <- mlb_analysis$hroin[[g]][1]
    shgame_df$hroinMLHSHARP[g] <- mlb_analysis$hroin[[g]][2]
    shgame_df$hroinSASHARP[g] <- mlb_analysis$hroin[[g]][3]
    shgame_df$hroinSHSHARP[g] <- mlb_analysis$hroin[[g]][4]
    phgame_df$hroinAPUBLIC[g] <- mlb_analysis$hroin[[g]][5]
    phgame_df$hroinHPUBLIC[g] <- mlb_analysis$hroin[[g]][6]
    phgame_df$hroinAMLPUBLIC[g] <- mlb_analysis$hroin[[g]][7]
    phgame_df$hroinHMLPUBLIC[g] <- mlb_analysis$hroin[[g]][8]
    mhgame_df$hroinAMONEY[g] <- mlb_analysis$hroin[[g]][9]
    mhgame_df$hroinHMONEY[g] <- mlb_analysis$hroin[[g]][10]
    mhgame_df$hroinAMLMONEY[g] <- mlb_analysis$hroin[[g]][11]
    mhgame_df$hroinHMLMONEY[g] <- mlb_analysis$hroin[[g]][12]
    
    shgame_df$mlhroiMLASHARP[g] <- mlb_analysis$mlhroi[[g]][1]
    shgame_df$mlhroiMLHSHARP[g] <- mlb_analysis$mlhroi[[g]][2]
    shgame_df$mlhroiSASHARP[g] <- mlb_analysis$mlhroi[[g]][3]
    shgame_df$mlhroiSHSHARP[g] <- mlb_analysis$mlhroi[[g]][4]
    phgame_df$mlhroiAPUBLIC[g] <- mlb_analysis$mlhroi[[g]][5]
    phgame_df$mlhroiHPUBLIC[g] <- mlb_analysis$mlhroi[[g]][6]
    phgame_df$mlhroiAMLPUBLIC[g] <- mlb_analysis$mlhroi[[g]][7]
    phgame_df$mlhroiHMLPUBLIC[g] <- mlb_analysis$mlhroi[[g]][8]
    mhgame_df$mlhroiAMONEY[g] <- mlb_analysis$mlhroi[[g]][9]
    mhgame_df$mlhroiHMONEY[g] <- mlb_analysis$mlhroi[[g]][10]
    mhgame_df$mlhroiAMLMONEY[g] <- mlb_analysis$mlhroi[[g]][11]
    mhgame_df$mlhroiHMLMONEY[g] <- mlb_analysis$mlhroi[[g]][12]
    
    shgame_df$mlhroinMLASHARP[g] <- mlb_analysis$mlhroin[[g]][1]
    shgame_df$mlhroinMLHSHARP[g] <- mlb_analysis$mlhroin[[g]][2]
    shgame_df$mlhroinSASHARP[g] <- mlb_analysis$mlhroin[[g]][3]
    shgame_df$mlhroinSHSHARP[g] <- mlb_analysis$mlhroin[[g]][4]
    phgame_df$mlhroinAPUBLIC[g] <- mlb_analysis$mlhroin[[g]][5]
    phgame_df$mlhroinHPUBLIC[g] <- mlb_analysis$mlhroin[[g]][6]
    phgame_df$mlhroinAMLPUBLIC[g] <- mlb_analysis$mlhroin[[g]][7]
    phgame_df$mlhroinHMLPUBLIC[g] <- mlb_analysis$mlhroin[[g]][8]
    mhgame_df$mlhroinAMONEY[g] <- mlb_analysis$mlhroin[[g]][9]
    mhgame_df$mlhroinHMONEY[g] <- mlb_analysis$mlhroin[[g]][10]
    mhgame_df$mlhroinAMLMONEY[g] <- mlb_analysis$mlhroin[[g]][11]
    mhgame_df$mlhroinHMLMONEY[g] <- mlb_analysis$mlhroin[[g]][12]
  }
  ...
  mlb_games_today <- action_all_df_today
  mlb_games <- list(sagame_df,
                    pagame_df,
                    magame_df,
                    shgame_df,
                    phgame_df,
