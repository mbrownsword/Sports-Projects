...
for(i in 1:length(years)){
  ...
  for(j in 1:length(months)){
    ...
    for(l in 1:length(days)){
      action_url <- paste0("https://api.actionnetwork.com/web/v1/scoreboard/nba?bookIds=123,75&date=",years[i],
...
nba_udf_analysis <- function(nba_all_games){
...
todays_nba_analysis <- function(action_all_df){
