...
nfl_backtest <- function(){
  ...
  for(i in 1:length(years)){
    ...
    for(j in 1:length(weeks)){
      action_url <- paste0("https://api.actionnetwork.com/web/v1/scoreboard/nfl?bookIds=123,75&season=",years[i],"&week=",
                           weeks[j])  
...
nfl_udf_analysis <- function(nba_all_games){
...
todays_nfl_analysis <- function(action_all_df, weeks, years){
  ...
  nfl_games_today <- action_all_df_today
  nfl_games <- list(sagame_df,
                    pagame_df,
                    magame_df,
                    shgame_df,
                    phgame_df,
