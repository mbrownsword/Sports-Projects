...
nhl_backtest <- function(){
  ...
  for(i in 1:length(years)){
    ...
    for(j in 1:length(months)){
      ...
      for(l in 1:length(days)){
        action_url <- paste0("https://api.actionnetwork.com/web/v1/scoreboard/nhl?bookIds=123,75&date=",years[i],
  ...
  nhl_all_games <- action_all_df
}

nhl_udf_analysis <- function(nba_all_games){
...
todays_nhl_analysis <- function(action_all_df){
  ...
  nhl_games_today <- action_all_df_today
  nhl_games <- list(sagame_df,
                    pagame_df,
                    magame_df,
                    shgame_df,
                    phgame_df,
