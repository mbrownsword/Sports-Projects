...
Is this where we're supposed to put all the code, and just have tables and the sort above? If so, I can reorganize before turning in the final draft.
...
plot(x= Softball_Stats2016$wOBA, y=Softball_Stats2016$PCT,
     xlab="wOBA", ylab = "PCT")
abline(lmwSoftballOBA2016,col="red")
...
season$runexp[season$base_state_before=="Runner on 2nd, One out"]<-.7709
season$runexp[season$base_state_before=="Runner on 2nd, Two outs"]<-.3006

season$runexp[season$base_state_before=="Runner on 3rd, No outs"]<-1.503
season$runexp[season$base_state_before=="Runner on 3rd, One out"]<-1.031
season$runexp[season$base_state_before=="Runner on 3rd, Two outs"]<-.413

season$runexp[season$base_state_before=="Runners on 1st and 2nd, No outs"]<-1.784
season$runexp[season$base_state_before=="Runners on 1st and 2nd, One out"]<-1.056
season$runexp[season$base_state_before=="Runners on 1st and 2nd, Two outs"]<-.456

season$runexp[season$base_state_before=="Runners on 1st and 3rd, No outs"]<-1.948
season$runexp[season$base_state_before=="Runners on 1st and 3rd, One out"]<-1.244
season$runexp[season$base_state_before=="Runners on 1st and 3rd, Two outs"]<-.496

season$runexp[season$base_state_before=="Runners on 2nd and 3rd, No outs"]<-2.089
season$runexp[season$base_state_before=="Runners on 2nd and 3rd, One out"]<-1.337
...
season$runexp[season$base_state_before=="Bases Loaded, One out"]<-1.505
season$runexp[season$base_state_before=="Bases Loaded, Two outs"]<-.618
...
run_exp <- c(.6705,.637,.675,.653,.6508,.6638,.647,.627,.6389,.6416,.903,.8071,.799, .813, 1.186, .8512)
...
"The Book: Playing the Percentages in baseball"_{1} starts with a handy first chapter aptly titled "Toolshed," in which it discusses fundamental tenants of statistical understanding necessary to evaluate trends and values in baseball. The accepted theory — as far as I have surmised on my admittedly-less-than-extensive search of the internet for advanced softball statistics — is that softball follows a pattern similar to baseball. 

This affects our understanding of softball. Statistics like wOBA, generated from MLB play data, have been used to explain the value of plays as if the sports were similar. This is the wOBA equation, with coefficients derived from the play's — i.e single, double, etc. — relative run value compared to the out. 

\[
\text{MLB's wOBA} \sim (.90*\text{Singles} + 1.24*\text{Doubles} + 1.56*\text{Triples} + 1.95*\text{Home runs} + .75*\text{Hit-By-Pitches} + .72*\text{Walks} +.90*\text{Reached Base on Error}) / \text{Plate Appearances} \]


In fact, a post_{2} on fastpitchanalytics.com closes with the author's desire to evaluate softball with different coefficients for the wOBA statistic specifically for softball.
...
\[
\text{Runs} \sim 1 + \text{Team wOBA} 
\]

\[
\text{Winning %} \sim 1 + \text{Team wOBA} 
\]
...
```{r}
library(xtable)
xtable(season[season$gameID==4032168&season$inningID==0,c("play","top_bottom","runs_before_play")])

xtable(season[season$gameID==4032168&season$inningID==0,c("play","top_bottom","runs_before_play")])

xtable(season[season$gameID==4032168&season$inningID==1,c("play","top_bottom","runs_before_play")])

xtable(season[season$gameID==4032168&season$inningID==2,c("play","top_bottom","runs_before_play")])


xtable(season[season$gameID==4032168&season$inningID==3,c("play","top_bottom","runs_before_play")])

xtable(season[season$gameID==4032168&season$inningID==4,c("play","top_bottom","runs_before_play")])
```
### Entire season
...
Based on these expectations, I expect to find that there will not only be differences in the constants in the wOBA expression, but also some variables will be weighted more than others differently than the MLB's wOBA. For example, if errors are going to be worth more, than the constant might be higher than singles, for example. 

\[
\text{MLB's wOBA} \sim (\text{Singles} + \text{Doubles} + \text{Triples} + \text{Home runs} +\text{Hit-By-Pitches} + \text{Walks} +\text{Reached Base on Error}) / \text{Plate Appearances} \]
...
\[
\text{Runs} \sim 1 + \text{Team wOBA}(\text{vs.})\text{Runs} \sim 1 + \text{Team mywOBA}
\]

\[
\text{Winning %} \sim 1 + \text{Team wOBA}(\text{vs.})\text{Winning %} \sim 1 + \text{Team myOBA} 
\]
...
I continued to do this for all of the base-out states, until I recorded all of them and created "Softball's Base-Out state" chart.^{13-}

```{r}
NoOutsS<-c(.656,1.1,1.395,1.503,1.784,1.948,2.089,2.294)
OneOutS<-c(.210,.603,.7709,1.031,1.056,1.244,1.337,1.505)
TwoOutsS<-c(.097,.23,.3006,.413,.456,.496,.566,.618)
baseoutstates
...
\[
\text{my wOBA} \sim (.69*\text{Singles} + 1.46*\text{Doubles} + 1.75*\text{Triples} + 2.30*\text{Home runs} + .90*\text{Hit-By-Pitches} + .71*\text{Walks} +1.01*\text{Reached Base on Error}) / \text{Plate Appearances} \]
...
plot<-ggplot(data = Softball_Stats2016, aes(x=myOBA,y=fit))+geom_line()+geom_ribbon(aes(ymin = ll, ymax = ul), col = "grey", alpha = 0.25) + labs(x = "myOBA", y = "Predicted Runs Scored")
plot

Softball_Stats2016$fit2 <- predict(lmwSoftballOBA2016R)
Softball_Stats2016$ul2 <- Softball_Stats2016$fit2 + 1.96 * predict(lmwSoftballOBA2016R, se.fit = T)$se
Softball_Stats2016$ll2 <- Softball_Stats2016$fit2 - 1.96 * predict(lmwSoftballOBA2016R, se.fit = T)$se
plot2<-ggplot(data = Softball_Stats2016, aes(x=wOBA,y=fit2))+geom_line()+geom_ribbon(aes(ymin = ll2, ymax = ul2), col = "grey", alpha = 0.25) + labs(x = "wOBA", y = "Predicted Runs Scored")
plot2
...
```{r}
lmmwSoftballOBA2015R
plot3<-ggplot(data = Softball_Stats2015, aes(x=myOBA,y=fit2))+geom_line()+geom_ribbon(aes(ymin = ll2, ymax = ul2), col = "grey", alpha = 0.25) + labs(x = "myOBA", y = "Predicted Runs Scored")
plot3

lmmwSoftballOBA2014R
plot4<-ggplot(data = Softball_Stats2014, aes(x=myOBA,y=fit2))+geom_line()+geom_ribbon(aes(ymin = ll2, ymax = ul2), col = "grey", alpha = 0.25) + labs(x = "myOBA", y = "Predicted Runs Scored")
plot4
```
...
The difference in results among the two charts in Softball and the MLB is staggering in both directions, as it is clear that having two outs is a much more dire circumstance in softball than in baseball. However, base-states with zero or one outs have almost uniformly higher metrics and almost all are significantly more. One theory behind this logic is that there are more runs scored per game in softball than in baseball, which seems to be true. 
...
There are four non-negative z statistics for a non-two out base-state, which are Bases Loaded, No outs; Nobody on, One out; Runners on 2nd and 3rd, One out and Bases Loaded, One out. This may signal decision-makers to place a heavier emphasis on getting the first batter on base, for example, or choosing to intentional walk good players more often in order to take advantage of common misconceptions about softball's similarity to baseball. 
...
The applications of this in the analysis section are well-documented and discussed, and the rsquared values are not only strong but very close to the baseball-optimized wOBA. However, this project is largely about the derivation of those constants and the function's applicability both for evaluating players and making decitions in games and during the course of a season. With that being said, my top three optimization opportunities in the game of softball based on these results: 
